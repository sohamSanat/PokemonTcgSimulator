...
// ── Master thematic pools shared across vendor tables (module scope so they can
// be reused by both the auction pools and the mixed trade/vault pool) ──────────
const vintageEng = [
...
// All curated (non-dynamic) English + vintage/modern Japanese master pools combined
const CURATED_MASTER_POOL = [
  ...vintageEng,
  ...vintageJpn,
  ...modernAlt,
  ...jpnModern,
...
// Determine whether a card is a Japanese-language card from any of its identifiers
export function isJapaneseVendorCard(c: { id?: string; setId?: string; name?: string; img?: string }): boolean {
  const id = (c.id || '') + '';
  const setId = (c.setId || '') + '';
  const name = (c.name || '') + '';
  const img = (c.img || '') + '';
  return (
    ...
    id.toLowerCase().includes('_ja') ||
    img.toLowerCase().includes('_ja') ||
    img.toLowerCase().includes('/ja/') ||
    name.toLowerCase().includes('japanese') ||
    name.toLowerCase().includes('(jpn)') ||
    name.toLowerCase().includes('kanji')
  );
}

// Best-effort extraction of a real setId / card number from an image URL (the most
// reliable source for the image fallback chain) with a fallback to parsing the id.
function parseSetAndNum(img: string, id: string): { setId: string; num: string } {
  if (img) {
    let m = img.match(/\/pokemon\/([a-z0-9_]+)-([0-9]+)(?:\/|$)/i);
    if (m) return { setId: m[1], num: m[2] };
    m = img.match(/pokemontcg\.io\/([a-z0-9]+)\/([0-9]+)/i);
    if (m) return { setId: m[1], num: m[2] };
  }
  if (id && id.includes('-')) {
    const idx = id.lastIndexOf('-');
    return { setId: id.slice(0, idx), num: id.slice(idx + 1) };
  }
  return { setId: 'swsh3', num: '1' };
}

/**
 * Returns a normalized, shuffled pool of vendor cards that mixes English & vintage
 * curated singles together with the thousands of dynamic Japanese catalog cards.
 * Each card carries a resolved setId/num so downstream views can run the shared
 * image fallback chain and never get stuck on a placeholder card-back.
 */
export function getCombinedVendorCardPool(dynamicCount: number = 4000): VendorCard[] {
  const dynamicJpn = getCardShowDynamicJapaneseCards(dynamicCount);

  const raw = [...CURATED_MASTER_POOL, ...dynamicJpn];
  ...
  return raw
    ...
    .map((c: any, i: number): VendorCard => {
      ...
      const parsed = (c.setId && c.num !== undefined)
        ...
        : parseSetAndNum(img, id);
      return {
        id,
        originalId: c.originalId || id,
        name: c.name || `Vendor Card #${i + 1}`,
        price,
        grade: c.grade || "Raw NM",
        img,
        setId: parsed.setId,
        num: parsed.num
      };
