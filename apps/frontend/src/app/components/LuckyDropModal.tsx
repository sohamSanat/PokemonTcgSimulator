...
import { Sparkles, Zap, Package, X, Layers } from 'lucide-react';
import { LuckyDropReward } from '../services/luckyDrop';

interface LuckyDropModalProps {
  isOpen: boolean;
  pack: LuckyDropReward | null;
  onClose: () => void;
  onOpenNow: (pack: LuckyDropReward) => void;
  onAddToInventory: (pack: LuckyDropReward) => void;
}
...
export const LuckyDropModal: React.FC<LuckyDropModalProps> = ({
  ...
  const isMystery = pack.type === 'mystery';
  const highlightSets = pack.mysteryPackConfig?.highlightSets;
          ...
          <div className="bg-gradient-to-br from-white/[0.07] to-white/[0.02] border border-amber-400/40 rounded-2xl p-4 mb-6 relative z-10 shadow-lg">
            <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              <div className="w-32 h-44 rounded-xl overflow-hidden bg-black/60 border border-amber-300/40 shrink-0 relative shadow-[0_10px_25px_rgba(0,0,0,0.7)] group">
                ...
                />
                <div className="absolute top-1.5 left-1.5 text-[10px] font-black bg-black/80 text-amber-300 px-2 py-0.5 rounded-md border border-amber-400/40 font-mono">
              ...
              <div className="flex-1 flex flex-col justify-between text-center sm:text-left">
                ...
                {isMystery && highlightSets && highlightSets.length > 0 && (
                  <div className="mt-3 pt-2 border-t border-white/10">
                    <span className="text-[10px] uppercase font-bold text-gray-400 block mb-1">
                      Featured Sets Inside:
                    </span>
                    <div className="flex flex-wrap gap-1 justify-center sm:justify-start">
                      {highlightSets.slice(0, 3).map((set, idx) => (
                ...
                {!isMystery && pack.seriesName && (
                  <div className="mt-3 pt-2 border-t border-white/10 flex items-center justify-center sm:justify-start gap-1.5 text-[11px] text-amber-300 font-semibold">
                    <Layers className="w-3.5 h-3.5 text-amber-400" />
                    <span>Generation: {pack.seriesName}</span>
                  </div>
                )}
              </div>
            </div>
