...
const FALLBACK_PRICES = {
  'Scarlet & Violet': 10.11,
  'Paldea Evolved': 15.50,
  'Obsidian Flames': 13.45,
  '151': 28.91,
  'Paradox Rift': 8.45,
  'Temporal Forces': 11.45,
  'Twilight Masquerade': 10.56,
  'Stellar Crown': 10.22,
  'Surging Sparks': 8.49,
  'Prismatic Evolutions': 15.99,
  'Journey Together': 6.45,
  'Destined Rivals': 9.36,
  'White Flare': 14,
  'Black Bolt': 15.45,
  'Shrouded Fable': 14.38,
  'Paldean Fates': 23.29,
  'Mega Evolution': 5.99,
  ...
  'Vivid Voltage': 10,
  'Shining Fates': 7.99,
  'Shining Fates Shiny Vault': 15.86,
  'Battle Styles': 8.49,
  'Chilling Reign': 11.78,
  'Evolving Skies': 44,
  'Fusion Strike': 19.39,
  'Brilliant Stars': 17.60,
  'Astral Radiance': 8.99,
  'Lost Origin': 19.00,
  'Silver Tempest': 14.29,
  'Crown Zenith': 25.50,
  'Pokemon Go': 8,
  'Celebrations': 41,
  'Champion\'s Path': 18.50
};
...
async function runScraper() {
  ...
  // 1. Save in scraper/data directory
  const backendFilePath = path.join(__dirname, '../data/set_pack_prices.json');
  fs.writeFileSync(backendFilePath, jsonContent, 'utf-8');
  console.log(`[✓] Saved to apps/scraper/data/set_pack_prices.json`);

  // 2. Save in apps/frontend directory so UI can import it directly
  const frontendDataDir = path.resolve(__dirname, '../../frontend/src/app/data');
  ...
  console.log(`[✓] Saved to apps/frontend/src/app/data/set_pack_prices.json`);
